...
async function loadUserStats() {
    try {
        ...
        if (xpElement)
            xpElement.textContent = `XP: ${data.experience || 0}`;
    }
    catch (error) {
...
async function loadStoryProgress() {
    try {
        const data = await apiClient.get('/story/progress');
        ...
        if (corruptionFill)
            corruptionFill.style.width = `${corruptionPercent}%`;
        if (corruptionText)
            corruptionText.textContent = `Corruption: ${corruptionPercent}%`;
    }
    catch (error) {
...
// Show mission detail
async function showMissionDetail(mission) {
...
async function loadMissionDialogue(level) {
    try {
        const data = await apiClient.get(`/story/missions/${level}/dialogues`);
...
function showDialogue(text) {
    ...
    if (ariaDialogue)
        ariaDialogue.classList.remove('hidden');
}
// Close dialogue
function closeDialogue() {
    const ariaDialogue = document.getElementById('aria-dialogue');
    if (ariaDialogue)
        ariaDialogue.classList.add('hidden');
}
// Display objectives
...
// Display rewards
function displayRewards(mission) {
...
// Create reward item
function createRewardItem(type, amount, icons) {
...
async function startMission() {
    ...
    if (missionNameElement)
        missionNameElement.textContent = currentMission.name;
    if (startModal) {
...
async function completeMission() {
    if (!currentMission)
        return;
    try {
        const data = await apiClient.post(`/missions/${currentMission.id}/complete`);
...
function showCompletionModal(data) {
    ...
    const rewardIcons = { 'gtk': '💰', 'experience': '⭐', 'materials': '⚙️' };
    ...
    if (data.unlock_feature) {
        ...
        if (unlockFeature)
            unlockFeature.textContent = formatFeatureName(data.unlock_feature);
    }
    else {
        if (unlockNotification)
