...
async function loadUserStats() {
    try {
        ...
        if (gtkElement)
            gtkElement.textContent = `GTK: ${data.gtk_balance || 0}`;
        if (levelElement)
...
async function loadRaidCount() {
    try {
        const data = await apiClient.get('/islands');
...
async function openIslandModal(island) {
    ...
    if (elements.name)
        elements.name.textContent = island.name;
    if (elements.biome)
        elements.biome.textContent = island.biome;
    if (elements.difficulty)
        elements.difficulty.textContent = island.difficulty;
    if (elements.entryFee)
        elements.entryFee.textContent = String(island.entry_fee);
    if (elements.reward)
        elements.reward.textContent = String(island.base_reward);
    if (elements.waves)
        elements.waves.textContent = String(island.wave_count);
...
// Load island details
async function loadIslandDetails(islandId) {
    try {
        const data = await apiClient.get(`/islands/${islandId}`);
...
// Display drop rates
function displayDrops(dropRates) {
...
async function enterRaid() {
    ...
    try {
        ...
        const data = await apiClient.post('/raids/start', { island_id: currentIsland.id, team_id: 1 });
...
function renderBattleUI() {
    ...
    if (currentWave)
        currentWave.textContent = String(raidData.current_stage || 1);
    if (totalWaves)
        totalWaves.textContent = String(raidData.total_stages || currentIsland?.wave_count || 1);
    if (playerHp)
        playerHp.textContent = String(raidData.current_team_hp);
...
async function performAction(actionType, moveSlot) {
    if (!raidData)
        return;
    ...
    try {
        const result = await apiClient.post(`/raids/${raidData.id}/turn`, {
...
async function endRaid(victory = false) {
    if (!raidData || !currentIsland)
        return;
    try {
        if (victory) {
            const data = await apiClient.post(`/raids/${raidData.id}/complete`, {});
...
// Show loot modal
function showLootModal(raid, rewards) {
    ...
    if (wavesElement)
        wavesElement.textContent = String(raid.current_stage || raid.waves_cleared || 0);
    if (hpElement)
        hpElement.textContent = String(raid.current_team_hp);
...
function setupEventListeners() {
    ...
    if (enterButton)
        enterButton.onclick = () => void enterRaid();
    if (endButton)
        endButton.onclick = () => void endRaid();
}
