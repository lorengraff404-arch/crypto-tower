...
export async function getAbilitiesByClass(className) {
    const response = await apiClient.get(`/abilities?class=${className}`);
    return response.abilities;
}
// Fetch learned abilities for a character
export async function getCharacterAbilities(characterId) {
    const response = await apiClient.get(`/characters/${characterId}/abilities`);
    return { learned: response.learned, available: response.available };
}
// Get type color for ability type
export function getAbilityTypeColor(type) {
    const colors = {
        'ACTIVE': '#3B82F6', // Blue
...
export function createAbilityCard(ability, isLearned = false) {
